<template>
    <div class="rich-text-editor">
      ...
      <div class="toolbar border border-gray-300 rounded-t bg-gray-50 p-1 flex flex-wrap gap-1">
        ...
        <!-- Media -->
        <div class="border-l border-gray-300 h-6 mx-1"></div>
        <button type="button" @click="insertVideo" title="Insert Video" class="p-1 rounded hover:bg-gray-200">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  // Use content prop if provided, otherwise use modelValue
  const initialContent = props.content || props.modelValue;
  
  if (initialContent) {
    editor.value.innerHTML = initialContent;
  } else if (props.placeholder && !isFocused.value) {
    editor.value.innerHTML = `<span class="editor-placeholder">${props.placeholder}</span>`;
  } else {
  ...
  const updatePlaceholder = () => {
    ...
    if (!props.modelValue && props.placeholder && !isFocused.value) {
      editor.value.innerHTML = `<span class="editor-placeholder">${props.placeholder}</span>`;
    } else if (!props.modelValue && !isFocused.value) {
      editor.value.innerHTML = '<p><br></p>';
    }
  };
  ...
  const handleFocus = () => {
    ...
    if (editor.value && editor.value.innerHTML.includes('editor-placeholder')) {
      editor.value.innerHTML = '<p><br></p>';
    }
    emit('focus');
  };
  ...
  const handleInput = () => {
    if (editor.value) {
      ...
      const content = editor.value.innerHTML
        .replace(/<span class="editor-placeholder">.*?<\/span>/g, '')
        .trim();
  ...
  const execCommand = (command, value = null) => {
    ...
    editor.value.focus();
    handleInput(); // Update model value after command
  };
  ...
  // Insert video
  const insertVideo = () => {
    const url = prompt('Enter the video URL:', 'https://');
    if (url) {
